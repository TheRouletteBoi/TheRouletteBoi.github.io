void main()
{
   ...
   var4 = null;
   var4 = Sidekick_AppendData(1, "SIDEKICK_TIMETRIAL_BEACHES_POST_TITLE", mcCareer_GetHookmanFlashId(var2), 21, 10.0f, var3, "Incoming Call!", "SIDEKICK_TIMETRIAL_BEACHES_POST", "", 3, 0, "SIDEKICK_TIMETRIAL_BEACHES_POST", var3, "PLAYER");
   while (!Sidekick_HasDataBeenAnnounced(var4))
   {
       WAITUNWARPED(200);
   }
   var5 = mcCareer_GetHookman(251);
   while (var5 != null)
   {
       WAITUNWARPED(500);
       var5 = mcCareer_GetHookman(251);
   }
   mcCareer_UnlockTimeTrial(1);
